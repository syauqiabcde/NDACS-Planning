...
percent_allowable_investment = 0.03
percent_allowable_om = 0.01
plant_lifetime = 30
nuclear_power_plant_lifetime = 60
water_consumption = 0 # in billion m3/Gton CO2/year              10.1016/j.spc.2025.02.017
water_production = 0.8 # in billion m3/Gton CO2/year              10.1016/j.cej.2024.154421
...
plant_capex = df['capex'].iloc[:,1].values * 1e9  # in $/Gton CO2/year
...
nuclear_capex = df['nuclear_capex'].iloc[:,1].values * 1e6  # in GW
...
def run_model(co2_scenario='IPCC middle', ppm_limit_scenario='1.5 C', optim_scenario=None):
    ...
    # Decision Variables
    model.new = Var(model.i, model.t, within=NonNegativeReals, bounds=(0.0, 0.8))   # new DACCS plant capacity in Gton CO2/year
    ...
    model.u = Var(model.i, model.t, model.h, within=NonNegativeReals)               # utilization of DACCS plant in Gton CO2/year, result of cf * capacity (used for avoiding bilinear term in objective function), although it has set of h, it is in unit of GtCOp2/year and in the captured co2 it itsdivided by the num-horus to reflect this change
    model.new_nuclear = Var(model.i, model.t, within=NonNegativeReals, bounds=(0.0, 500.0)) # new nuclear power plant capacity in GW
    ...
    # Expressions
    def IPCC_prediction(t, scenario):        # return the CO2 concentration in ppm based on IPCC prediction 
        if scenario == 'IPCC conservative':
            return 0.03707 * t**2 - 145.53282 * t + 143098.86396
        elif scenario == 'IPCC middle':
            return 0.01152 * t**2 - 42.91808 * t + 40081.81896
        elif scenario == 'IPCC optimistic':
            return -0.01403 * t**2 + 59.69666 * t - 62935.22605
    ...
    def system_om_cost(model, i, t):
        daccs_fix_om = model.capex[t] * model.capacity[i,t] * fixed_om           # Fixed OM cost of Nuclear DACCS plant
        nuclear_fix_om = model.nuclear_capex[t] * model.nuclear_capacity[i,t] * nuclear_fixed_om                     # Nuclear power plant investment cost
    ...
    def system_water_consumption(model, i, t):
        return sum(model.captured_co2[i,t,h] * (water_consumption - water_production) for h in model.h) + model.nuclear_capacity[i,t] * nuclear_water_consumption
    ...
    # Objective Function
    def tot_cost(model):
        return sum(((model.capex[t] * model.new[i,t]                                      # DACCS investment cost
                    + model.nuclear_capex[t] * model.new_nuclear[i,t]                     # Nuclear power plant investment cost
    ...
    def financing_constraint(model, i, t):
        return (model.new[i, t] * model.capex[t] + model.nuclear_capex[t] * model.new_nuclear[i,t]) <= percent_allowable_investment * model.gdp[i, t] * 1e6
    ...
    if result.solver.termination_condition == 'optimal':
        ...
        for x, i in enumerate(model.i):
            for y, t in enumerate(model.t):
                ...
                result_dict['Investment Required'][x,y] = value((model.capex[t] * model.new[i,t]) + (model.nuclear_capex[t] * model.new_nuclear[i,t])) / 1e9   # Investment required in billion USD
