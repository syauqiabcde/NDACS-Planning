...
data_sheet = 'data_5years.xlsx'
wb = xl.load_workbook(data_sheet) 
sheet_name = wb.sheetnames
df = {sheet: pd.read_excel(data_sheet, sheet_name=sheet) for sheet in sheet_name}
...
co2_storage_capacity = df['co2_storage_capacity'].iloc[:,1].values   # from giga ton to ton
...
# Decision Variables
model.new = Var(model.i, model.t, within=NonNegativeReals, bounds=(0.0, 1.0 * 1e3))    # new capacity unit ton CO2/year, very high upper bound to avoid infeasibility
...
model.u = Var(model.i, model.t, model.h, within=NonNegativeReals) # u is variable subtituting cf * capacity to make the model linear, unit ton CO2/year
...
def captured_co2(model, i, t, h):   # hourly CO2 captured
    return model.u[i,t,h] * interval / num_hours 

model.retired = Expression(model.i, model.t, rule=retirement)           # retired plant capacity in ton CO2/year
model.capacity = Expression(model.i, model.t, rule=capacity_expansion)  # plant capacity in ton CO2/year
model.captured_co2 = Expression(model.i, model.t, model.h, rule=captured_co2)   # captured CO2 in ton CO2
...
def tot_cost(model):
    ...
    return sum(((model.capex[t] * model.new[i,t] * 1e9                        # Investment cost
                + model.capex[t] * model.capacity[i,t] * 1e9 * fixed_om       # Fixed OM cost of Nuclear DACCS plant
                + model.energy_duty[i,h] * 1000 * model.captured_co2[i,t,h] * 1e9 * (heat_share * heat_cost + (1-heat_share) * elc_cost))  # energy cost, multiply 1000 due to conversion from MJ/kg to MJ/ton
                - model.co2_tax[i, t] * model.captured_co2[i,t,h] * 1e9)        # CO2 tax benefit
                / (1+discount_rate)**(t-min(model.t))                     # Discount rate  
                for i in model.i for t in model.t for h in model.h)       
...
def upper_u(model, i, t, h):
    return model.u[i, t, h] <= model.capacity[i, t]
