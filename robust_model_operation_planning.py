...
# Capacity optimization result
path = f'Result/result {case_name}.xlsx'
wb_capacity = xl.load_workbook(path) 
sheet_name_capacity = wb_capacity.sheetnames
df_capacity = {sheet: pd.read_excel(path, sheet_name=sheet) for sheet in sheet_name_capacity}
new_capacity = df_capacity['New Capacity'].iloc[:,1:].values 
capacity_opt = df_capacity['Capacity'].iloc[:,1:].values
...
# Decision Variables
model.co2_ppm = Var(model.t, within=NonNegativeReals, bounds=(0.0, 600.0))       # not really a decision variables just a way to determine the co2 ppm based on the previous value of co2 ppm
model.cf = Var(model.i, model.t, model.h, within=NonNegativeReals, bounds=(0.0, 1.0))
...
def captured_co2(model, i, t, h):   # hourly CO2 captured
    return model.capacity[i,t] / num_hours * model.cf[i,t,h] * interval * 1000000
...
def tot_cost(model):
    return sum(((model.capex[t] * model.new[i,t]                        # Investment cost
                ...
                + model.energy_duty[i,h] * 1000 * model.captured_co2[i,t,h] * (heat_share * heat_cost + (1-heat_share) * elc_cost))  # energy cost, multiply 1000 due to conversion from MJ/kg to MJ/ton
                - model.co2_tax[i, t] * model.captured_co2[i,t,h])        # CO2 tax benefit
...
def rampup_constraint(model, i, t, h):
    if h == min(model.h):
        ...
        return (model.cf[i, t, h] - model.cf[i, t, h-1]) <= 0.1
    
def rampdown_constraint(model, i, t, h):
    if h == min(model.h):
        return Constraint.Skip
    else:
        return (model.cf[i, t, h-1] - model.cf[i, t, h]) <= 0.1
...
#                         solve_io="nl")
result = solver.solve(model, tee=True, options={"nopresol": ""})
...
if tc in (TerminationCondition.infeasible, TerminationCondition.other, TerminationCondition.unbounded, TerminationCondition.infeasibleOrUnbounded):
...
#     "Capacity Factor": np.zeros((num_countries, num_periods, num_hours)),
...
#                 result_dict['Capacity Factor'][x,y,z] = model.cf[i,t,h].value
...
#     with pd.ExcelWriter(f"Result/result {case_name} dispatch.xlsx") as writer:
